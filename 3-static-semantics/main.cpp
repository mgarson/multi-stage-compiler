...
// These tokens are then passed to the parser, which uses the project grammar to create the parse tree. A symbol table is then created
// and a function is called to check the static semantics of the tree and add valid variables to the symbol table. The symbol table with
// all declared variables is then printed.
